...
class Scene(object):
	...
	def enter(self):
		print "This function isn't made not yet. To inherit and make enter()."
		exit(1)

class Engine(object):
	
	def __init__(self, scene_map):
		print "Engine __init__'s scene_map", scene_map
		self.scene_map = scene_map
	
	def play(self):
		current_scene = self.scene_map.opening_scene()
		print "Play of the first scene", current_scene
		while True:
			print "\n-------------"
			...
			if next_scene_name != 'finished':
				...
				print "New scene was reiceved from the map.", current_scene
			else:
...
class Death(Scene):
	...
	quips = [
		"Death. I can't do this game anymore.",
		"My mother proud me...if I was smater..",
		"like a looser..",
		"My puppy that I raise it is better that you."
		]
...
class CentralConrridor(Scene):
	...
	def enter(self):
		...
		print "이어지는 사악한 광대 복장의 고던인이 띄쳐 나오는 동안 당신은 중앙"
		...
		if action == "fire":
			print "당신은 관선총을 빼들기가 무섭게 고던인을 쏘아버립니다. 광대옷이 날려"
			...
			print "하나 합니다."
			print "Lasfl askjdfhkj askdjfh askj wqzdf werf wer sdf gfg wer dfg wer sfd xcv,"
			print "sdf sdf qwe ghj rty vnb hgf yrtdf dfg."
...
class LaserWeaponArmory(Scene):
	...
	def enter(self):
		...
		print "합니다. 비밀먼호를 10번 틀리면 자물쇠는 영원히 잠기고 폭탄을 꺼낼 수"
		print "없습니다. 비밀번호는 3자리수입니다."
		code = "%d%d%d" % (randint(1,9), randint(1, 9), randint(1,9))
		print "code: %d" % int(code)
		guesses = 0
		guess = raw_input("[[%d] Board of Number]>" %guesses)

		while guesses != code and guesses < 9:
			print "Pbbbbb!"
			guesses += 1
			guess = raw_input("[[%d] Board of Number]>" %guesses)
...
class EscapePod(Scene):
	...
	def enter(self):
		...
		print "good_pod: %d" % int(good_pod)
		guess = raw_input("[Pod's number]>")
		...
		if int(guess) != good_pod:
			...
			print "%s번 구명정으로 뛰어들어 탈출 단추를 누릅니다."
			print "구명정을 가볍게 우주로 미끄러져 나가며 아래의 행성으로 향합니다."
...
class Map(object):
	...
	scenes = {
		'central_corridor': CentralConrridor(),
	...
	def __init__(self, start_scene):
		...
		print "__init__'s start_scene", self.start_scene
		
	def next_scene(self, scene_name):
		print "next_scene's start_scene"
		val = Map.scenes.get(scene_name)
		print "next_scene's result is ", val
		return val
